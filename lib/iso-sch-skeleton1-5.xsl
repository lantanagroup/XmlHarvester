...
<xsl:template match="sch:schema | schema">
	<axsl:stylesheet version="1.0">
		...
		<axsl:template match="/">
			<xsl:call-template name="process-root">
				...
				<xsl:with-param 	xmlns:sch="http://purl.oclc.org/dsdl/schematron"
	...
	or (../phase[@id= ($phase)]/active[@id= current()/@id])">
		<xsl:call-template name="process-pattern">
		    <!-- Rick Geimer commented out the name attribute to use the title element per iso schematron -->
		    <!--
			<xsl:with-param name="name" select="@name"/>
			-->
			<xsl:with-param name="name" select="sch:title | title"/>
			<xsl:with-param name="id" select="@id"/>
